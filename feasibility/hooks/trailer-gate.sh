...
deny() { echo "feasibility: refused — $1" >&2; exit 2; }
...
FEAS_PAYLOAD="$payload" \
FEAS_CPD="${CLAUDE_PROJECT_DIR:-}" \
FEAS_CWD="$(pwd -P 2>/dev/null || echo)" \
python3 <<'PY'
import json, os, posixpath, re, shlex, subprocess, sys

def deny(msg):
    sys.stderr.write("feasibility: refused — %s\n" % msg)
    sys.exit(2)
...
def _feas_fail_closed(_t, _v, _tb):
    try:
        sys.stderr.write("feasibility: refused — fail-closed: internal error (%s: %s)\n" % (_t.__name__, _v))
    except Exception:
        pass
    os._exit(2)
sys.excepthook = _feas_fail_closed
...
cwd = os.environ.get("FEAS_CWD", "") or "."
...
if [ "$rc" -ne 0 ] && [ "$rc" -ne 2 ]; then
  echo "feasibility: refused — fail-closed: internal error (gate judge exited $rc)" >&2
  exit 2
fi
exit "$rc"
