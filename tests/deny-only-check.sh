...
# --- substance probe: an empty feasibility record must be refused ---------
probe_dir="${1:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../feasibility/hooks" && pwd -P)}"
rec_rel="docs/issue-999/reports/feasibility.md"
...
substance_probe() {
  ...
  rm -rf "$td"
  if [ "$refused" = 0 ]; then
    echo "deny-only-check: FAIL — no gate refuses an empty feasibility record at $rec_rel (contract s20)" >&2
    return 1
  fi
  return 0
}
